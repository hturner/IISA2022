...
.footer[&nbsp;&nbsp; [github.com/hturner/IISA2022](https://github.com/hturner/IISA2022)]
...
Use `bench::mark()` to test your changes. Create a wrapper to run a specified move square function `n` times with different seeds:
```{r, eval = FALSE}
run <- function(n, fun){
  x <- numeric(n)
  for (i in seq_len(n)) {
...
```
Then run `bench::mark(run(n, move_square), run(n, move_square2))` for `n = 1000`.

When you have done as much improvement as you can, compare `profvis(run(n, move_square))` with `profvis(run(n, move_square2))`.
...
https://www.meetup.com/en-AU/chennai-r-user-group/

![Creating an R Package: Why and How? Speaker: Dr Heather Turner. Date: Friday 6 Jan 2023. Hybrid: Zoom + OMR, Chennai (TBD). 6-7pm lecture, 7-8pm networking and refreshments. Special thanks to our sponsors Princeps Technologies Inc](chennai_meetup.png)
