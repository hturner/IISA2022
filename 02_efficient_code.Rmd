...
# Faster functions for common operations
...
# Case Study (h/t Raju Bhakta)

Consider the following code to generate random values from the mixture distribution 0.3 × N(0, 1) + 0.5 × N(10, 1) + 0.2 × N(3, 0.1)
...
We can use vectorization to avoid this for loop!
...
Use this information to replace the for loop on the last slide, using a single call to `rnorm` to simulate `n` values from the mixture distribution.
